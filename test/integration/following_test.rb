...
class FollowingTest < ActionDispatch::IntegrationTest
  ...
  test 'should unfollow a user the standard way' do
    @user.follow(@other)
    relationship = @user.active_user_relationships.find_by(followed_id: @other.id)
    assert_difference '@user.following.count', -1 do
  ...
  test 'feed on Home page' do
    get root_path
    @user.feed.paginate(page: 1).each do |bookpost|
      assert_match CGI.escapeHTML(bookpost.content), response.body
    end
  end
end
